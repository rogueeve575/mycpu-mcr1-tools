...
uchar *args, *ptr;
	...
	//printf("is_function_definition: '%s'\n", line);
	...
	// quick hack to omit lambda functions when { is on the next line
//	if (strstr(line, "[]"))
//		return 0;
	...
	if ((ptr = strchr(line, '{')))
	{
		return 0;	// hack to prevent picking up functions in structs in C files
		/*
		*ptr = 0;
		--ptr;
		while(ptr >= line)
	...
	// they don't contain any semicolons anywhere in them
	// (culls some multiline for loops)
	if (strchr(line, ';'))
		return 0;
...
char is_reserved_keyword(char *str)
{
	...
	if (No_Staterr && !strcmp(str, "staterr")) return 1;
...
int nfuncs;
	...
	// don't include macros from e.g. wxWindows
	//printf("'%s'\n", funcname);
	if (string_all_uppercase(funcname))
	{
		//printf("not adding all-uppercase function '%s'\n", funcname);
		return 0;
	}
...
char string_all_uppercase(const char *str)
{
int i;
