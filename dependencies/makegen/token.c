...
{
ml_header_section *localsect = (src>=0) ? sources[src].hdrsec : header->defsec;
token *t;
int i;
char *catstr;
char prefix_binpath;
	...
	while(t)
	{
		...
		switch(t->type)
		{
			...
			case TKN_OBJ_EXT: catstr = localsect->obj_ext; break;
			case TKN_CC: catstr = localsect->cc; break;
			case TKN_HOSTFLAGS: catstr = header->defsec->hostflags; break;
...
static uchar GetVarType(char *vname)
{
	...
	if (!strcasecmp(vname, "CC")) return TKN_CC;
	if (!strcasecmp(vname, "HOSTFLAGS")) return TKN_HOSTFLAGS;
